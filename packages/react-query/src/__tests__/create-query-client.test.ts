...
test.each([
  ...
  'createQueryClient returns DevupQueryClient instance: %s',
  (baseUrl) => {
...
test.each([
  ...
  'createQueryClient accepts api with defaultOptions: %s',
...
test.each([['openapi.json'], ['openapi2.json']] as const)(
  'createQueryClient accepts api with serverName: %s',
  (serverName) => {
    const api = createApi({
